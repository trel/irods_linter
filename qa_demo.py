...
def main():
    ...
    checks = [
        ...
        # Type checking
        (".venv/bin/python -m mypy linter/ --ignore-missing-imports", "Running Type Checking"),
        
        # Security scanning
        (".venv/bin/python -m bandit -r linter/ -f json", "Running Security Scan"),
        
        # Coverage check
        (".venv/bin/python -m pytest tests/ --cov=linter --cov-fail-under=80 --quiet", "Checking Test Coverage"),
        
        # Example configurations
        (".venv/bin/python irods_linter.py examples/secure_unattended_installation.json", "Testing Secure Configuration"),
        
        # JSON validation
        ("python -c \"import json; [json.load(open(f)) for f in ['examples/sample_unattended_installation.json', 'examples/secure_unattended_installation.json']]\"", "Validating JSON Examples"),
    ]
